...
# Python 3.9
# Visualizing indels by Plotting the events on the tree
...
from ete3 import PhyloTree, Tree, TreeStyle, NodeStyle, faces, AttrFace, CircleFace, TextFace, SeqGroup, SequenceFace
import numpy as np
from panel.widgets import Tqdm
from tree import IndelTree


class PlottingEvents(IndelTree):
    def __init__(self, tr, initializer, tqdm):
        ...
        for node in tr.traverse("postorder"):
            if node not in tr.iter_leaves():
                a = node.children[0]
                node.name = initializer.dict_relation.get(a.name)
                # print("The list of internal nodes:", node.name)
            initializer.lst_names.append(node.name)
        # print("The list of all nodes:", initializer.lst_names)

        align_fas = format(initializer.aligned_data, 'fasta')

        # Making a directory to write the files:
        directory = "EventsPerSite/"
        path = os.getcwd()
        path_w_dir = os.path.join(path, directory)
        ...
        # render_site(1150, tr, aln) # one site 1150 ,1188, 235,

        self.render_all_sites(tr, initializer.aligned_data, initializer.mat_insertion, initializer.mat_deletion,
                              initializer.dic_name_index, tqdm)  # all the sites in the msa file

        print("Successfully change the directory back to %s " % path)
        os.chdir(path)

    def link_to_align(self, tree, aln):
        """
        Connecting the MSA characters to the phylotree object. It is the same function as in ete3.
        :param tree: The ete3 phylotree.
        :param aln: The alignment file which should be linked with the tree.
        :return:
        """
        missing_leaves = []
        missing_internals = []
        ...
        for n in tree.traverse():
            try:
                ...
                if n.is_leaf():
                    ...
                    missing_internals.append(n.name)
        ...
        if len(missing_internals) > 0:
            print("Warnning: [%d] internal nodes could not be found in the alignment." % \
                  len(missing_leaves), file=sys.stderr)

    def render_site(self, site_number, tree, alignment, mat_insertion, mat_deletion, dict_name_index):
        """
        Saving the tree and msa column for one site in "png" format.
        :param site_number: The number of the selected site.
        :param tree: The ete3 phylotree.
        :param alignment: The alignment file which should be linked with the tree.
        """
        single_col_str = self.get_single_residue_msa(alignment, site_number)
        ...
        lst_child = IndelTree.get_father_subtree_of_node(self, tree)
        self.reset_view(tree)
        IndelTree.event_on_tree(self, tree, ins_event, del_event, dict_name_index, lst_child)
        #     for n in tree.traverse():
    ...
    def render_all_sites(self, tree, alignment, mat_insertion, mat_deletion, dict_name_index, tqdm):
        """
        Saving the tree and msa columns for all the existing sites in "png" format.
        :param tree: The ete3 phylotree.
        :param alignment: The alignment file which should be linked with the tree.
        :param mat_insertion: Matrix of the insertion events of size(number of sites*number of taxa) 0/1.
        :param mat_deletion: Matrix of the deletion events of size(number of sites*number of taxa) 0/1.
        :param dict_name_index: A Dictionary of 'value:index'.
        :param tqdm: A progress bar of the panel.
        """
        aln_len = alignment.get_alignment_length()
        for i in tqdm(range(0, aln_len)):
            self.render_site(i, tree, alignment, mat_insertion, mat_deletion, dict_name_index)
        print("All the sites were printed successfully.")

    def get_single_residue_msa(self, alignment, site_number):
        """
        ...
        :return: single_res: A single column MSA file containing chars.
        """
        single_res = ""
        sites_count = 0
    ...
    def reset_view(self, tree):
        """
        Delete all the features and styles from the last run, since the style and feature are attached to the tree.
        :param tree: The ete3 phylotree.
        :return:
        """
        for n in tree.traverse():
    ...
    def print_view(self, tree):
        """
        This function meant to apply events on the tree topology. The layout function could not change the style,
